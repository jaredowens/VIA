...
export default function PersonalEditPage() {
  ...
  const returnToSelf = useMemo(() => `/setup/${cardId}/edit/personal`, [cardId]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!sessionData.session) {
        window.location.href = `/login?returnTo=${encodeURIComponent(returnToSelf)}`;
        return;
      }
  ...
  }, [cardId, returnToCard, returnToSelf]);
  ...
  // Only show phone + email on this page (but we keep full actions in state so nothing gets lost)
  const contactTypes: ActionType[] = ["phone", "email"];
  const contacts = actions.filter((a) => contactTypes.includes(a.type));
      ...
      <div className="relative flex min-h-screen items-center justify-center px-6 py-10">
        <div className="w-full max-w-[560px] rounded-[28px] border border-white/10 bg-[#121214]/80 p-10 shadow-[0_30px_120px_rgba(0,0,0,0.75)] backdrop-blur-xl">
          ...
          <div className="text-center">
            <h1 className="text-lg font-medium tracking-wide text-white/90">Edit Personal Info</h1>
          ...
          {loading ? (
            ...
            <div className="mt-10 space-y-8">
              ...
              {/* CONTACT */}
              <div className="space-y-4">
                <div className="text-xs tracking-[0.35em] text-white/45">CONTACT</div>
                    ...
                    <div key={a.id} className="rounded-2xl border border-white/12 bg-white/5 p-4">
                      ...
                      <div className="mt-3">
                        <label className="block text-xs tracking-wider text-white/55 mb-2">
                          {a.type === "phone" ? "PHONE NUMBER" : "EMAIL"}
                        </label>
                        <input
                          value={a.value}
                      ...
                      {a.type === "phone" && (
                        <p className="mt-2 text-xs text-white/40 tracking-wide">Required. On card: tap to message, hold to copy.</p>
                      )}
                      {a.type === "email" && (
                        <p className="mt-2 text-xs text-white/40 tracking-wide">Optional. On card: tap copies it.</p>
                      )}
                    </div>
                  ))}
                </div>
