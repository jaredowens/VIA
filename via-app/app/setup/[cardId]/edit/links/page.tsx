...
export default function LinksEditPage() {
  ...
  const returnToSelf = useMemo(() => `/setup/${cardId}/edit/links`, [cardId]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        .from("cards")
        .select("id, owner_user_id, payments_json")
        .eq("id", cardId)
        .maybeSingle();
  ...
  async function save() {
    ...
    try {
      ...
      // Keep phone/email in the blob (do NOT require them on this page)
      const phone = cleaned.find((x) => x.type === "phone")?.value ?? "";
...
window.location.href = next === "card" ? returnToCard : returnToCard;
  ...
  const paymentTypes: ActionType[] = ["venmo", "cashapp", "paypal"];
  const payments = actions.filter((a) => paymentTypes.includes(a.type));
  const external = actions.filter((a) => !paymentTypes.includes(a.type) && a.type !== "phone" && a.type !== "email");
      ...
      <div className="relative flex min-h-screen items-center justify-center px-6 py-10">
        <div className="w-full max-w-[560px] rounded-[28px] border border-white/10 bg-[#121214]/80 p-10 shadow-[0_30px_120px_rgba(0,0,0,0.75)] backdrop-blur-xl">
          ...
          <div className="text-center">
            <h1 className="text-lg font-medium tracking-wide text-white/90">Edit Payments & Links</h1>
